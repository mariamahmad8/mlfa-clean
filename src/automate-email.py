...
def classify_email(subject, body):
    ...
    **"jail_mail" is a SPECIALIZED SUBSET of legal-related communication, but it is a DISTINCT AND EXCLUSIVE ROUTING CATEGORY.**
    ...
    IMPORTANT:
    - Determine "jail_mail" using ONLY the current message (ignore prior thread context)
    ...
    **Active communications** → Categorize as `"active_communication"` for legitimate, non-spam email threads or replies that do not fit any other defined category but are still relevant and meaningful to MLFA.
    ...
    If none of the above apply, classify as `"irrelevant_other"`
    ...
    CONVERSATION STAGE OVERRIDE (CRITICAL):
